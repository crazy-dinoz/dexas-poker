      ...
      msg=JSON.parse(msg);
       ...
       socket.on(self.room+'moneyChanged',(msg)=>{
        cc.log(msg);
           ...
           console.log('moneyChanged'+msg.name);
            if(msg.name==self.player1Name.string){
              if(msg.num>10000){
...
if(msg.name==self.player8Name.string){
  ...
  socket.on('paipaipai'+self.room,(msg)=>{
      ...
      //  this.genzhunum.string="跟注"+255;//用这个来显示用户要跟注的筹码
         ...
         cc.log(data);
          ...
          if(self.allmoney>self.jiazhu){
            ...
            if(data.id== self.userid){
                ...
                if(self.num<2&&self.jiazhunum.string!='all in'){
        ...
        socket.on('youxianzhu'+self.room,(data)=>{
            data=JSON.parse(data);
            self.lun=1;
            self.damang=2*data.money;
            self.jiazhu=self.damang;
            
        });
        socket.on('youxian'+self.room,(data)=>{
            data=JSON.parse(data);
            self.num=data.lunnum1;
            console.log(self.lun+'youxian里面的lun');
            if(self.lun<3&&self.num<=3){
                self.jiazhu+=self.damang;
            
             }else if(self.lun>2&&self.num<=3)
                {
                    self.jiazhu+=2*self.damang;
                }
        });
       socket.on(m.name,(data)=>{
        ...
        socket.on(self.room+'jieshu',function(data){
            ...
            if(self.userid==data.id&&data.statusa==66){
                ...
                console.log('恭喜你获得了奖金'+data.num);
                    ...
                    cc.sys.localStorage.setItem("UserMessage",m);
        ...
        socket.on(self.userid+'one',function(data){
            ...
            self.p7.opacity = 255;
            self.p8.opacity = 255;
            self.action1Label.string='';
            self.action2Label.string='';
            self.action3Label.string='';
            self.action4Label.string='';
            self.action5Label.string='';
            self.action6Label.string='';
            self.action7Label.string='';
            self.action8Label.string='';
            self.dichi.string='底池：0';
            console.log(data.zhuang+"zhuangtai"+"zhuang"+data.zhuang+"uid+"+data.uid);
            self.id=data.uid;
    ...
    callback: function (event) {
      ...
      var jiazhu=(self.allmoney-2*self.money)*self.slider.progress;
      self.jiazhunum.string='Raise'+(parseInt(2*self.money)+parseInt(jiazhu));
      self.jiazhu=(parseInt(2*self.money)+parseInt(jiazhu));
    ...
    buttonjiazhu:function(){
        ...
        socket.emit('youxian',{roomid:self.room,lunnum:self.num});
        self.yincang();
    },
