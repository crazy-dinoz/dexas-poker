...
router.get('/regist', function (req, res) {
  ...
  if (phone == null || yzm == null || passwd == null || phone == "" || yzm == "" || passwd == "" 
  //|| cp.status != true
  ) {
    returnmsg.status = 1;
    returnmsg.msg.desc = "Invalid parameter upload";
    res.send(JSON.stringify(returnmsg));
    return;
  };
  reclient.hget("phone", phone, function (err, reply) {
    console.log("get phone : "+reply);
    //先判断该手机号是不是获取过验证码
    if (reply == null || reply == "null") {
      returnmsg.status = 1;
      returnmsg.msg.desc = "Please get verification code first";
      res.send(JSON.stringify(returnmsg));
    } else {//查看验证码是否过期
      reply = reply.split("/");
      ...
      if (everdate < 600) {
        if (reply[0] == yzm) {
          ...
          reclient.hget("userphone", phone, function (err, reply) {
            ...
            if (reply != null) {
              returnmsg.status = 1;
              returnmsg.msg.desc = "Số điện thoại này đã được đăng ký";
              res.send(JSON.stringify(returnmsg));
              return;
            };
...
//////////////////
// thêm ngay sau khi insert vào register thành công
var sql_adduser = sqlcmd.Insert({
  User_Id: uid,
  Name: phone,
  Password: passwd,
  Phone: phone,
...
sqlcmd.Doit(sql_adduser, function (err, vals, field) {
  if (err) console.log("❌ Error insert user:", err);
  else console.log("✅ User inserted to 'user' table:", vals);
});
...
//////////////////
          ...
          returnmsg.status = 1;
          returnmsg.msg.desc = "Mã xác minh không đúng";
          res.send(JSON.stringify(returnmsg));
        }
      }
      else {
        returnmsg.status = 1;
        returnmsg.msg.desc = "Mã xác minh đã hết hạn";
...
router.get('/forgetpas',function (req,res){
 ...
 // var tuijian=req.query.tuijian;
  ...
  if (phone == null || yzm == null || passwd == null || phone == "" || yzm == "" || passwd == "" || cp.status != true) {
    returnmsg.status = 1;
    returnmsg.msg.desc = "Lỗi tham số tải lên";
    res.send(JSON.stringify(returnmsg));
    return;
  };
  reclient.hget("phone", phone, function (err, reply) {
    ...
    if (reply == null || reply == "null") {
      returnmsg.status = 1;
      returnmsg.msg.desc = "Vui lòng lấy mã xác minh";
      res.send(JSON.stringify(returnmsg));
    } else {//查看验证码是否过期
      reply = reply.split("/");
      var everdate = (new Date().getTime() - Number(reply[1])) / 1000;
      if (everdate < 600) {
        if (reply[0] == yzm) {
            ...
            sqlcmd.Doit(sqlstr,function(qerr,vals,field){
                   if(qerr==null){
                     returnmsg.status=0;
                     returnmsg.msg.desc='Sửa đổi thành công';
                      res.send(JSON.stringify(returnmsg));
                   }else{
                     returnmsg.status=1;
                     returnmsg.msg.desc='Sửa đổi không thành công';
                      res.send(JSON.stringify(returnmsg));
                   }
            })
        }
...
router.get('/login', function (req, res) {
 ...
 var returnmsg = { status: 0, msg: { Userid: 0, desc: "", name: "", sex: "", portrait: "",lv:0,exp:0,scoin:0,gcoin:0,yb:0,zs:0,power:0,drink:"" } };
  //先判断验证码
  var cp=checkPhone.checkMobile(phone);
  if (phone == null || passwd == null || phone == "" || passwd == "" 
   // || /*(/^0?1[0-9][0-9]\d{8}$/.test(phone))*/ cp.status!= true
  ) {
    returnmsg.status = 2;
    returnmsg.msg.desc = "Upload parameter error";
    console.log("login...Upload parameter error")
    res.send(JSON.stringify(returnmsg));
    return;
  };
  var sha1 = crypto.createHash('sha1');
  ...
  sqlcmd.Doit(sqlstr, function (qerr, vals, filed) {
    ...
    if (vals.length == 0) {
      returnmsg.status = 1;
      returnmsg.msg.desc = "sername and password do not match";
      res.send(JSON.stringify(returnmsg));
    }
    else {
...
router.get('/isPhone', function (req, res) {
  var phone = req.query.phone;
  console.log("isPhone");
  if ((/^0?9[0-9][0-9]\d{8}$/.test(phone)) != true) {
    res.send('{"status":"2"}');
    return;
  };
  reclient.hget('userphone', phone, function (err, reply) {
